#Developed by @knownliar on telegram
...
class automator:
    ...
    def update_ascii(self):
        options = f"""
{self.console_msg("1")} Follower Bot {Fore.WHITE}{self.status["followers"]}
{self.console_msg("2")} Like Video Bot {Fore.WHITE}{self.status["likes"]}
{self.console_msg("3")} View Bot {Fore.WHITE}{self.status["views"]}
{self.console_msg("4")} Share Bot {Fore.WHITE}{self.status["shares"]}
        """
    ...
    def update_title(self, bot, remaining, rl = False):
        if clear == "cls":
            os.system(clear)
            ctypes.windll.kernel32.SetConsoleTitleW(f"TikTok AIO | Sent: {self.sent} | Cooldown: {remaining}s | Developed by @knownliar on telegram")
            print(ascii_text)
            ...
            if rl:
                ...
                cooldown = remaining
            print(self.console_msg(self.cooldowns) + f" Cooldowns {Fore.WHITE}[{Fore.WHITE}{cooldown}s{Fore.WHITE}]")
            print(self.console_msg(self.ratelimits) + f" Ratelimits {Fore.WHITE}[{Fore.WHITE}{rl_cooldown}s{Fore.WHITE}]")

    def main(self):
        if clear == "cls":
            ctypes.windll.kernel32.SetConsoleTitleW("TikTok AIO | Developed by @knownliar on telegram")
    ...
    def start(self):
        ...
        try:
            option = int(input(f"                {Fore.WHITE}> {Fore.WHITE}"))
        except ValueError:
            self.start()
        if option == 1:
